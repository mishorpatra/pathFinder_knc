...
var scannedTag = window.location.search.split('&&mar=')[1]
...
class IndoorNavigation extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      showModal: false,
  ...
  srcChange = e => {
    ...
    if (e.length > 0) {
      ...
      this.props.imgDetails(
        ...
        () => {
          console.log("imgdetails",this.props.getImage)
          // this.set
          this.setState(
            {
              srcName: name,
  ...
  componentDidMount() {
    ...
    try {
      ...
      var markerVal = window.location.href.split("=")[1];
      ...
      this.props.floorList(this.buildingName, () => {
        ...
        if(sensor.hasReading === false){
          this.actualAngle = this.floorAngle[0]
        }
    ...
    if (this.buildingName) {
      ...
      this.props.getAllElements(() => {
        ...
        for (var i of get_keys) {
          ...
          console.log("fc",fc,"keys",get_keys,"values",get_values,i)
          var cnt = 0;
          var prev = null;
        ...
        if (
          ...
          for (let i = 0; i < nodes.length; i++) {
            if (markerVal === nodes[i].arValue) {
              let ind = this.find_floor(nodes[i].floor, this.props.fList);
              // // var utter = new window.SpeechSynthesisUtterance(nodes[i].name);
      ...
      if (srcfloor === dstfloor) {
        ...
        this.setState(
          ...
          async () => {
            ...
            // (async () => {
              console.log("img load",img1,img1.onload)
  ...
  async callwhile(srcfloor, srcVal, m, n, dest_x, dest_y, single, canvasid) {
    ...
    if (single == true) {
      ...
      if (this.state.destination && (this.state.zoom === false)) {
  ...
  refresh = () => {
    if(scannedTag) this.props.history.push("/cmb/wayfinding?KNCollege")
    ...
    return (
      <div style={{ backgroundColor: "#f7f7f7" }}>
        ...
        >
          ...
          >
            <i className="fa fa-camera text-white" style={{ fontSize: "1em" }}></i>
          </button> */}
          {!this.state.samefloor ? (
                <div
                  className="btn-group-vertical zoom "
                  role="group"
                ...
                >
                  {this.state.next ? (
                    <button
                      type="button"
                      className="btn btn-info btn-sm text-capitalize text-white"
                      id="next"
                      data-tip
                      data-for="next"
              ...
              <p className="navbar-brand mx-auto h5 text-info font-weight-bolder ml-5">
          {t(`College Main Building`)} 
          </p>
        </nav>
        {/* {this.state.selectedProperty === "Search" ? ( ) : null} */}
          <div className="row w-100 ml-0">
            <div className="col-12 ">
              <div className="m-2 d-flex-column">
                <form>
                  <div>
                    <Typeahead
                      ...
                      onFocus={()=>{this.setState({ srcReadOnly: false });}}
                      onBlur={(e) => {
                        this.setState({ srcReadOnly: false });
                      }}
                  ...
                  <div>
                    <Typeahead
                      ...
                      onBlur={(e) => {
                        this.setState({ dstReadOnly: false });
                      }}
                      onFocus={()=>{this.setState({ dstReadOnly: false });}}
...
<div
            className="btn-group-vertical bg-info shadow grp-btn mr-1"
          ...
          >
            ...
            >
              <i className="fa fa-home" style={{ fontSize: "1em" }}></i>
            </button>            
              <button
            ...
            >
              <i className="fa fa-refresh" style={{ fontSize: "1em" }} />
            </button>
       
          </div>
          <Link
              to="/ar"
              className="navbar-brand mx-auto text-white btn btn-info btn-circle btn-xl font-weight-bolder mr-2 cam-btn"
              title="Scan nearby tags"
            >
                <i className="fa fa-camera mt-1" style={{fontSize:"24px"}}></i>
            </Link>
          ...
          <div>
            <div className="mt-1 canvaDiv">
              ...
              <img
                alt="map"
                id="srcImage"
                src="/knc/assets/images/start.png"
                hidden={true}
              ></img>
              <img
                alt="map"
                id="dstImage"
                src="/knc/assets/images/destination.png"
                hidden={true}
              ></img>
              <img
                alt="map"
                id="stairs"
                src="/knc/assets/images/stair.png"
                hidden={true}
              ></img>
              <img
                alt="map"
                id="lift"
                src="/knc/assets/images/lift.png"
                hidden={true}
              ></img>
          ...
          >
            {this.state.loaded ? (
              <div className="container d-flex-column justify-content-center text-center">
                <p
                  className="h5  font-weight-bolder ml-5"
                  style={{ color: "#4782bc" }}
                >
                </p>
              </div>
